...
class main{
            ...
            if(!empty($user) AND !empty($pw)) {
                ...
                if($sql->rowCount()) {
                    ...
                    if($data['role'] == "admin") {
                        ...
                        $_SESSION['role'] = "admin";
                        header("Refresh:0; url=home_admin.php?menu=absen");
                        ...
                        $_SESSION['id'] = $data['id'];
                        $_SESSION['nama'] = true;
                        header("Refresh:0; url=home_siswa.php?menu=absen");
                    }
                }else{
    ...
    public function tampiladmin($db,$tab){
        $go = $this->_db->prepare("SELECT * FROM $tab");
        $go->execute();
    ...
    public function simpan($con, $tabel, array $field, $redirect){
    ...
    public function simpan2($con, $tabel2, array $field2, $redirect){
    ...
    public function tampilAbsen($db, $tabel, $where){
        $go = $this->_db->prepare("SELECT * FROM $tabel where  $where");
        $go->execute();
